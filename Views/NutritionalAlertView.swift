...
struct NutritionalAlertView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Text(NSLocalizedString("Nutrition Insights", comment: "Header title for nutritional alert"))
                    .font(.headline)
                    .foregroundColor(headerColor)
            ...
            ScrollView(.vertical, showsIndicators: true) {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    Button(action: {
                        ...
                        HStack {
                            Text(NSLocalizedString("View Full Analysis", comment: "Button to view detailed nutrition analysis"))
                            Image(systemName: LocalizationHelper.forwardChevron)
                        }
    ...
    private func criticalWarningsSection(warnings: [HealthWarning]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(NSLocalizedString("Health Considerations", comment: "Section title for health warnings"))
                .font(.headline)
                .foregroundColor(.red)
    ...
    private var positiveEffectsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(NSLocalizedString("Beneficial Nutrients", comment: "Section title for positive nutrients"))
                .font(.headline)
            ...
            if analysisResult.positiveEffects.count > 2 {
                Text(String(format: NSLocalizedString("... and %d more benefits", comment: "Message showing number of additional benefits"), analysisResult.positiveEffects.count - 2))
